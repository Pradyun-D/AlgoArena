...
if (Array.isArray(sidebarUser.registered_rounds) && sidebarUser.registered_rounds.length > 0) {
    registeredRoundsList.innerHTML = sidebarUser.registered_rounds.map((round) => `
        <div class="flex items-center gap-3">
            <div class="w-1 h-8 ${round.is_live ? "bg-secondary" : "bg-primary"} rounded-full"></div>
            <div>
                <p class="text-[11px] font-bold uppercase leading-tight">${escapeHtml(round.title)}</p>
                <p class="text-[9px] text-on-surface-variant">${escapeHtml(round.subtitle)}</p>
            </div>
        </div>
    `).join("");
} else {
    registeredRoundsEmpty.classList.remove("hidden");
}
